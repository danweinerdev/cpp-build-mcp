...
### Subtasks
- [ ] Add `TargetsRequested []string \`json:"targets_requested,omitempty"\`` to `buildResponse` struct
- [ ] In `handleBuild`, set `resp.TargetsRequested = targets` (the variable is already nil when no targets provided)
- [ ] Unit test: `json.Marshal` of `buildResponse{TargetsRequested: nil}` does not contain `targets_requested`
- [ ] Unit test: `json.Marshal` of `buildResponse{TargetsRequested: []string{"app"}}` contains `"targets_requested":["app"]`
- [ ] Verify existing build handler tests still pass (response shape is a superset)
...
### Subtasks
- [ ] Remove `mcp.WithArray("targets", ...)` from the `clean` tool registration
- [ ] Remove the targets extraction block from `handleClean` (lines that extract `rawTargets`)
- [ ] Pass `nil` to `inst.builder.Clean(ctx, nil)` explicitly
- [ ] Update any tests that reference the clean tool's targets parameter
...
### Subtasks
- [ ] At the top of `handleClean` (after `resolveConfig`), check `inst.store.GetPhase() < state.PhaseConfigured`
- [ ] If unconfigured, return `mcp.NewToolResultError("Project not configured. Call configure() first.")`
- [ ] Unit test: new store (phase Unconfigured) → clean returns tool error
- [ ] Unit test: store after SetConfigured → clean succeeds
- [ ] Verify `store.SetClean()` is only called after a successful clean on a configured project
...
### Subtasks
- [ ] Check if `state.Store` already has a `GetPhase()` or equivalent method
- [ ] If not, add `GetPhase() Phase` that acquires read lock and returns `s.state.Phase`
- [ ] Unit test: returns `PhaseUnconfigured` initially, `PhaseConfigured` after `SetConfigured`
...
- [ ] `go vet ./...`
- [ ] `go test -race ./...`
- [ ] Review any test failures from schema changes (clean targets removed, buildResponse shape change)
- [ ] Confirm all pre-existing tests pass

## Acceptance Criteria

- [ ] `build(targets: ["app"])` response includes `"targets_requested": ["app"]`
- [ ] `build()` (no targets) response does not contain `targets_requested` key
- [ ] `clean` tool schema has only `config` parameter
- [ ] `clean` on unconfigured project returns tool error
- [ ] `clean` on configured project succeeds and calls `SetClean`
